...
function indekseGoreFenomen(fenomenler,index) {
 ...
 let text= index + ". indekste bulunan fenomen: " + x
 return text
...
function fenomenSil(gelenDizi,indeks) {
...
console.log(fenomenEkle(fenomenler, 1250, "Şirzat", 240, 22, "İnstagram"));
...
function fenomenGonderimSayisi(arrayOfObject, fenomenName){
  let fenomenPostSayisi;
  for(let i =0; i<arrayOfObject.length; i++){
    if(fenomenName === arrayOfObject[i].profile){
...
function platformaGoreCokGonderiYapanFenomen(arrayOfObject, platform){
  let feno;
  let max =0;
  for(let i = 0; i<arrayOfObject.length; i++){
    console.log(i);
    if(arrayOfObject[i].platform === platform){
      if(arrayOfObject[i].posts !== "NA" && arrayOfObject[i].posts > max ){
        max = arrayOfObject[i].posts;
        feno = arrayOfObject[i].profile;
      }
    }
  }
  return feno;
